...
def create_ijbonddict(eType, eId):
    ...
    return i_jbond_dict
# =========================================
def create_fingerprints(nType, i_jbond_dict, radius):
...
def get_PYG_DGL(dataset):
    ...
    for i, graph in enumerate(dataset):
        try:
            ...
            graphs_PYG_DGL.append((g_PYG, g_DGL, graph.yTrue, graph.morgan, graph.maccs))
        except:
            continue
...
def get_pretrained_embedding(model, graphs):
    ...
    graphs_MolCLR, graphs_MSGNN, labels, morgan, maccs = list(zip(*graphs))

    h_list, out_list, graph_list, y_list, morgan_list, maccs_list = [], [], [], [], [], []
    batch_size = 32
    ...
    return h_list, out_list, graph_list, y_list, morgan_list, maccs_list
# =============================================
class Graph():
...
class MolGraph():
    ...
    def get_graph(self, data, aug_num = 0, train=True):
        ...
        try:
            ...
            sf = create_fingerprints(nf[:, :11], i_jbond_dict, radius)
    ...
    def mask_nodes_edges(self, graph, M, N):
        ...
        for atom_idx in mask_nodes:
            nf_new[atom_idx, :] = np.zeros(graph.nf[atom_idx].shape) # 连续值不需要用0再代替了，因为这已经全初始化为0了  'formalCharge'，'aromatic'，'mass'，'NumRadicalElectrons'
        ...
        for bond_idx in range(2 * M):
            if bond_idx not in mask_edges_1:
                ...
                count += 1
        graph_new = Graph(graph.smi, nf_new, edge_attr_new, edge_index_new, graph.yTrue, graph.x_CLR, graph.edge_attr_CLR,
...
def create_cross_data(ori_train, ori_test, fold, aug_num=0):
    ...
    for data in ori_test:
        try:
            graph = molGraph.get_graph(data, train=False)
            if graph is not None:
                test_dataset.append(graph)
        except:
            continue

    graphs_train = get_PYG_DGL(list(train_dataset))
    ...
    h_train, out_train, graph_train, y_train, morgan_train, maccs_train = get_pretrained_embedding(model, graphs_train)
    h_test, out_test, graph_test, y_test, morgan_test, maccs_test = get_pretrained_embedding(model, graphs_test)
